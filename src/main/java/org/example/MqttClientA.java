...
//        String broker = "tcp://localhost:1883";
        ...
        int qos = 0;
        String sendTopic = "topic/client_b/" + homeTopic;
...
//            logger.log(Level.INFO, "Connected to broker: " + broker)
        ...
        mqttClient.publish(sendTopic, "Connected".getBytes(), qos, false);
...
//                    System.out.println("Message received: " + mqttMessage.toString() + " from " + topic.split("/")[2]);
                        ...
                        if (mqttMessage.toString().equals("c") || mqttMessage.toString().equals("1")) {
                            OnDataReceiveListener onDataReceiveListener = data -> {
//                                System.out.println(data);
                                Thread thread = new Thread(() -> {
                                    try {
                                        mqttClient.publish(sendTopic, data.getBytes(), qos, false);
                                    } catch (Exception e) {
                                        e.printStackTrace();
                                    }
                                });
